...
HTTP Basic Auth is enforced by both Caddy hops for every path except
`/healthz`. Configure the protected Woodpecker secrets `searx_auth_user` and
`searx_auth_hash` (a bcrypt hash generated by Caddy); only the username and
hash are stored in root-only runtime files, so credentials survive redeploys
and reboots. Rotate those secrets and rerun the generated SearX secret
workflows. Keep Cloudflare abuse controls and the follower firewall enabled;
Basic Auth alone is not rate limiting.
The plaintext password is held only by the operator/browser password manager;
the server and CI retain the username and bcrypt hash.
