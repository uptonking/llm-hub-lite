...
`searx:8080` service. Its persistent config and cache live below
`data/prod/searx`; `SEARX_SECRET` is generated and stored only in the selected
follower's `/etc/llm-hub-lite/searx.env`. The app has no host-published port,
Valkey sidecar, Redis, or Docker socket and is capped at 900 MiB, 0.80 CPU,
and 128 processes. SearXNG's limiter and public-instance mode are disabled in
the minimal profile, with Cloudflare and the follower firewall providing the
edge controls. Change `NODES` in `config/cluster/apps/searx.policy` or run
