...
grep -q '^APP_ID=aichor$' "$repo_root/apps/aichor/manifest.env"
grep -q '^APP_ID=aichor3$' "$repo_root/apps/aichor3/manifest.env"
grep -q '^UPSTREAM_MODE=singleton$' "$repo_root/apps/aichor3/manifest.env"
grep -q '^DATA_ROOT_REL=aichor3$' "$repo_root/apps/aichor3/manifest.env"
grep -q '^NODE_SECRET_KEYS=AICHOR3_PASSWORD,GH_TOKEN$' "$repo_root/apps/aichor3/manifest.env"
grep -q '^OPTIONAL_SECRET_KEYS=GH_TOKEN$' "$repo_root/apps/aichor3/manifest.env"
grep -Fq 'GH_TOKEN:GH_TOKEN' "$repo_root/apps/aichor3/manifest.env"
grep -q '^GENERATED_SECRET_BYTES=AICHOR3_PASSWORD:32$' "$repo_root/apps/aichor3/manifest.env"
grep -q '^AICHOR3_MEMORY_LIMIT=900m$' "$repo_root/apps/aichor3/config.env"
grep -q '^AICHOR3_CPUS=0.80$' "$repo_root/apps/aichor3/config.env"
grep -q '^AICHOR3_PIDS_LIMIT=128$' "$repo_root/apps/aichor3/config.env"
grep -Fq 'PASEO_PASSWORD: ${AICHOR3_PASSWORD:?AICHOR3_PASSWORD must be set}' "$repo_root/apps/aichor3/compose.yml"
grep -Fq 'GH_TOKEN: ${GH_TOKEN:-}' "$repo_root/apps/aichor3/compose.yml"
grep -Fq 'mem_limit: ${AICHOR3_MEMORY_LIMIT:-900m}' "$repo_root/apps/aichor3/compose.yml"
grep -Fq 'cpus: ${AICHOR3_CPUS:-0.80}' "$repo_root/apps/aichor3/compose.yml"
grep -Fq '/aichor3:/home/paseo' "$repo_root/apps/aichor3/compose.yml"
grep -Fq 'reverse_proxy aichor3:6767' "$repo_root/apps/aichor3/route.follower.caddy"
grep -Fq '{$AICHOR3_SITE}' "$repo_root/apps/aichor3/route.leader.caddy"
grep -q '^ENABLED=true$' "$repo_root/config/cluster/apps/aichor3.policy"
grep -q '^NODES=worker-3$' "$repo_root/config/cluster/apps/aichor3.policy"
grep -q '^NODE_AICHOR3_ORIGIN_HOST=worker3-aichor3-origin.aichorage.de$' "$repo_root/config/cluster/nodes/worker-3.env"
grep -q '^APP_ID=searx$' "$repo_root/apps/searx/manifest.env"
grep -q '^UPSTREAM_MODE=singleton$' "$repo_root/apps/searx/manifest.env"
grep -q '^STATE_MODE=files$' "$repo_root/apps/searx/manifest.env"
grep -q '^NODE_SECRET_KEYS=SEARX_SECRET$' "$repo_root/apps/searx/manifest.env"
grep -q '^CLUSTER_SECRET_KEYS=SEARX_AUTH_USER,SEARX_AUTH_HASH$' "$repo_root/apps/searx/manifest.env"
grep -q '^DEPLOYMENT_SECRET_KEYS=SEARX_AUTH_USER,SEARX_AUTH_HASH$' "$repo_root/apps/searx/manifest.env"
grep -q '^GENERATED_SECRET_BYTES=SEARX_SECRET:32$' "$repo_root/apps/searx/manifest.env"
grep -q '^SECRET_REGEXES=SEARX_AUTH_USER:' "$repo_root/apps/searx/manifest.env"
grep -q '^SEARX_MEMORY_LIMIT=900m$' "$repo_root/apps/searx/config.env"
grep -q '^SEARX_CPUS=0.80$' "$repo_root/apps/searx/config.env"
...
grep -Fq 'health_uri /healthz' "$repo_root/apps/searx/route.leader.caddy"
grep -Fq 'basic_auth @searx_auth_required' "$repo_root/apps/searx/route.leader.caddy"
grep -Fq 'not path /healthz' "$repo_root/apps/searx/route.leader.caddy"
grep -Fq 'basic_auth @searx_auth_required' "$repo_root/apps/searx/route.follower.caddy"
grep -Fq 'not path /healthz' "$repo_root/apps/searx/route.follower.caddy"
grep -q '^NODES=worker-2$' "$repo_root/config/cluster/apps/searx.policy"
grep -q '^NODE_SEARX_ORIGIN_HOST=worker2-searx-origin.aichorage.de$' "$repo_root/config/cluster/nodes/worker-2.env"
if grep -Eiq 'valkey|redis|ports:|/var/run/docker.sock' "$repo_root/apps/searx/compose.yml"; then
	printf 'SearX must remain a single container without Valkey, host ports, or Docker socket\n' >&2
	exit 1
fi
grep -Fq '../paseo/entrypoint-common.sh:/paseo-entrypoint-common.sh:ro' "$repo_root/apps/aichor/compose.yml"
grep -Fq '../paseo/entrypoint-common.sh:/paseo-entrypoint-common.sh:ro' "$repo_root/apps/aichor3/compose.yml"
grep -q '^UPSTREAM_MODE=singleton$' "$repo_root/apps/aichor/manifest.env"
grep -q '^STATE_MODE=files$' "$repo_root/apps/aichor/manifest.env"
grep -q '^RECOVERY_REQUIRED_PATHS=.paseo/server-id,.paseo/daemon-keypair.json,.paseo/config.json$' "$repo_root/apps/aichor/manifest.env"
grep -q '^RUNTIME_ENV_PROVISION=stage$' "$repo_root/apps/aichor/manifest.env"
grep -q '^NODE_SECRET_KEYS=AICHOR_PASSWORD,GH_TOKEN$' "$repo_root/apps/aichor/manifest.env"
grep -q '^OPTIONAL_SECRET_KEYS=GH_TOKEN$' "$repo_root/apps/aichor/manifest.env"
grep -Fq 'GH_TOKEN:GH_TOKEN' "$repo_root/apps/aichor/manifest.env"
grep -q '^CONDITIONAL_SECRET_KEYS=AICHOR_PI_OPENAI_ENABLED=true|AICHOR_PI_OPENAI_API_KEY;AICHOR_PI_ANTHROPIC_ENABLED=true|AICHOR_PI_ANTHROPIC_API_KEY;AICHOR_PI_OPENROUTER_ENABLED=true|AICHOR_PI_OPENROUTER_API_KEY$' "$repo_root/apps/aichor/manifest.env"
grep -Fq 'AICHOR_PASSWORD:aichor_password' "$repo_root/apps/aichor/manifest.env"
...
grep -q '^AICHOR_GH_VERSION=[0-9]\+\.[0-9]\+\.[0-9]\+$' "$repo_root/images/aichor/release.env"
grep -q '^AICHOR_GH_SHA256=[0-9a-f]\{64\}$' "$repo_root/images/aichor/release.env"
...
grep -Fq 'AICHOR_GH_VERSION' "$repo_root/images/aichor/Dockerfile"
grep -Fq 'sha256sum --check' "$repo_root/images/aichor/Dockerfile"
grep -Fq 'command -v gh' "$repo_root/images/aichor/Dockerfile"
...
grep -Fq 'PASEO_CORS_ORIGINS: ${AICHOR_CORS_ORIGINS:-${AICHOR_SITE:?AICHOR_SITE must be set}}' "$repo_root/apps/aichor/compose.yml"
grep -Fq 'GH_TOKEN: ${GH_TOKEN:-}' "$repo_root/apps/aichor/compose.yml"
grep -Fq './entrypoint.sh:/aichor-entrypoint.sh:ro' "$repo_root/apps/aichor/compose.yml"
grep -Fq 'AICHOR_PI_OPENAI_API_KEY: ${AICHOR_PI_OPENAI_API_KEY:-}' "$repo_root/apps/aichor/compose.yml"
grep -Fq 'AICHOR_PI_ANTHROPIC_API_KEY: ${AICHOR_PI_ANTHROPIC_API_KEY:-}' "$repo_root/apps/aichor/compose.yml"
grep -Fq 'AICHOR_PI_OPENROUTER_API_KEY: ${AICHOR_PI_OPENROUTER_API_KEY:-}' "$repo_root/apps/aichor/compose.yml"
...
grep -Fq '.aichor-ownership-v1' "$repo_root/apps/aichor/entrypoint.sh"
grep -Fq 'PASEO_APP_PREFIX=AICHOR' "$repo_root/apps/aichor/entrypoint.sh"
grep -Fq 'chown -R 1000:1000 "$home/.paseo" "$home/.pi" "$daemon_home"' "$repo_root/apps/paseo/entrypoint-common.sh"
grep -Fq 'reverse_proxy aichor:6767' "$repo_root/apps/aichor/route.follower.caddy"
grep -Fq 'tls internal' "$repo_root/apps/librechat/route.follower.caddy"
grep -Fq 'tls internal' "$repo_root/apps/wapdf/route.follower.caddy"
grep -Fq 'tls_insecure_skip_verify' "$repo_root/apps/librechat/route.leader.caddy"
grep -Fq 'tls_insecure_skip_verify' "$repo_root/apps/wapdf/route.leader.caddy"
grep -Fq 'tls internal' "$repo_root/apps/aichor/route.follower.caddy"
grep -Fq 'tls internal' "$repo_root/apps/aichorouter/route.follower.caddy"
grep -Fq 'tls internal' "$repo_root/apps/cpapi/route.follower.caddy"
grep -Fq 'tls internal' "$repo_root/apps/cursorapi/route.follower.caddy"
grep -Fq 'flush_interval -1' "$repo_root/apps/aichor/route.follower.caddy"
grep -Fq 'header_up Host {$AICHOR_SITE_HOST}:443' "$repo_root/apps/aichor/route.follower.caddy"
grep -Fq 'header_up X-Forwarded-Host {$AICHOR_SITE_HOST}' "$repo_root/apps/aichor/route.follower.caddy"
grep -Fq 'header_up X-Forwarded-Proto https' "$repo_root/apps/aichor/route.follower.caddy"
grep -Fq 'health_uri /api/health' "$repo_root/apps/aichor/route.leader.caddy"
grep -Fq 'tls_insecure_skip_verify' "$repo_root/apps/aichor/route.leader.caddy"
grep -Fq 'tls_insecure_skip_verify' "$repo_root/apps/aichorouter/route.leader.caddy"
grep -Fq 'tls_insecure_skip_verify' "$repo_root/apps/cpapi/route.leader.caddy"
grep -Fq 'tls_insecure_skip_verify' "$repo_root/apps/cursorapi/route.leader.caddy"
grep -Fq 'tls internal' "$repo_root/apps/flowy/route.follower.caddy"
...
if grep -Eq 'header_up (Upgrade|Connection)' "$repo_root/apps/aichor/route.leader.caddy" "$repo_root/apps/aichor/route.follower.caddy"; then
	printf 'Aichor routes must rely on Caddy native WebSocket upgrade handling\n' >&2
	exit 1
fi
grep -q '^NODES=worker-2$' "$repo_root/config/cluster/apps/aichor.policy"
grep -q '^NODE_AICHOR_ORIGIN_HOST=worker2-aichor-origin.aichorage.de$' "$repo_root/config/cluster/nodes/worker-2.env"
