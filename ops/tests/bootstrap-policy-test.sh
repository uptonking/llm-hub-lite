...
grep -Fq 'NODE_BACKUP_ENABLED="$(sed -n' "$bootstrap"
grep -Fq 'systemctl disable --now platform-backup.timer platform-backup-prune.timer platform-backup-check.timer' "$bootstrap"
grep -Fq 'if [[ "$NODE_BACKUP_ENABLED" == true ]]; then' "$bootstrap"
