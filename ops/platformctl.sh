...
singleton_runtime_env_ready() {
	...
	[[ -f "$(app_runtime_env_file "$d")" ]] && return 0
...
validate_descriptor() {
	...
	*) die "unsupported UPSTREAM_MODE in $d/manifest.env" ;;
	...
	if app_in_reconcile_scope "$d" && singleton_runtime_env_ready "$d" && [[ "$PLATFORM_TEST_SKIP_COMPOSE_INSPECTION" == 0 ]]; then
		app_compose "$d"
	...
	sqlite)
		...
		if app_in_reconcile_scope "$d" && singleton_runtime_env_ready "$d"; then
			while IFS= read -r k; do
				[[ -n "$k" ]] || continue
	...
	*) die "unsupported STATE_MODE in $d/manifest.env" ;;
	...
	if [[ "$(app_upstream_mode "$d")" == singleton && "$(app_in_reconcile_scope "$d" && printf true || printf false)" == true && "$(singleton_runtime_env_ready "$d" && printf true || printf false)" == true ]]; then
		while IFS= read -r k; do
			[[ -n "$k" ]] || continue
			value="$(app_value "$d" "$k")"
