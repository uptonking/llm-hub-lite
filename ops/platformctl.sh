...
PLATFORM_VALIDATE_LOCAL="${PLATFORM_VALIDATE_LOCAL:-0}"
...
validate_test_flags() {
	...
	[[ "$PLATFORM_VALIDATE_LOCAL" == 0 || "$PLATFORM_VALIDATE_LOCAL" == 1 ]] || die 'PLATFORM_VALIDATE_LOCAL must be 0 or 1'
...
singleton_runtime_env_provisioned() {
...
singleton_runtime_env_ready() {
	...
	# creates node-local secrets, but only when every secret is generated by the
	# target node. Operator-supplied or Leader-owned secrets must already exist.
...
validate_descriptor() {
	...
	*) die "unsupported INGRESS_MODE in $d/manifest.env" ;;
	...
	if [[ -n "$rel" ]]; then
		safe_relative "$rel" || die "unsafe DATA_ROOT_REL in $d/manifest.env"
	...
	active-passive)
		...
		;;
	singleton)
		[[ -n "$(descriptor_value "$d" MOVE_MODE)" ]] || die "MOVE_MODE is required for singleton app $d/manifest.env"
		if [[ -z "$(descriptor_value "$d" RUNTIME_ENV_FILE)" ]]; then
			[[ -z "$(descriptor_value "$d" CLUSTER_SECRET_KEYS)" && -z "$(descriptor_value "$d" NODE_SECRET_KEYS)" && -z "$(descriptor_value "$d" CONDITIONAL_SECRET_KEYS)" ]] || die "singleton without runtime env cannot declare secrets: $d/manifest.env"
		fi
	...
	*) die "unsupported UPSTREAM_MODE in $d/manifest.env" ;;
	...
	if app_in_reconcile_scope "$d" && ([[ -z "$(descriptor_value "$d" RUNTIME_ENV_FILE)" ]] || singleton_runtime_env_provisioned "$d") && [[ "$PLATFORM_TEST_SKIP_COMPOSE_INSPECTION" == 0 ]]; then
		app_compose "$d"
	...
	sqlite)
		...
		if app_in_reconcile_scope "$d" && singleton_runtime_env_provisioned "$d"; then
			while IFS= read -r k; do
				[[ -n "$k" ]] || continue
...
validate() {
	...
	need_file "$APP_ENV"
	if [[ "$PLATFORM_TEST_FAST_VALIDATE" == 0 && "$local_scope" == 0 ]]; then
...
singleton_prepare() {
	...
	if [[ "$(descriptor_value "$d" STATE_MODE)" == ephemeral && -z "$(descriptor_value "$d" DATA_ROOT_REL)" ]]; then
		transition_begin "$journal" "$(basename "$d")" "$previous" "$target" "$release" ""
		transition_set "$journal" PHASE prepared
		rm -f -- "$state_file"
		printf 'prepared stateless singleton %s without persistent data\n' "$(basename "$d")"
		return 0
	fi
	base="$(data_root)"
	rel="$(descriptor_value "$d" DATA_ROOT_REL)"
...
EOF
	...
	# newly-issued origin certificates. The Host header still exercises Paseo's
	# configured hostname policy; the Leader-side public smoke validates Caddy
...
singleton_stop() {
	...
	rm -f -- "$state_file"
	runtime_env="$(app_runtime_env_file "$d")"
	if [[ -z "$(descriptor_value "$d" DATA_ROOT_REL)" ]]; then
		printf 'stopped stateless singleton %s; no persistent data retained%s\n' "$(basename "$d")" "${runtime_env:+; runtime secrets retained at $runtime_env}"
		return 0
	fi
	base="$(data_root)"
	rel="$(descriptor_value "$d" DATA_ROOT_REL)"
