...
apply_snapshot() {
  ...
  for path in "$DATA_ROOT" "$APP_ROOT/shared/runtime" "$APP_ROOT/shared/.env.prod" "$APP_ROOT/current" "$APP_ROOT/previous" \
    ...
    "$CONFIG_ROOT/platform.env" "$CONFIG_ROOT/images.apps.env" "$CONFIG_ROOT/images.foundation.env" "$CONFIG_ROOT/beszel-initial-credentials"; do
    if ! swap_path "$target$path" "$path" "$rollback_root" "$manifest"; then
      rollback_swaps "$manifest"; PLATFORM_LOCK_HELD=1 "$PLATFORMCTL_SCRIPT" start all || true; die 'restore swap failed; prior paths restored; maintenance mode retained for verification'
    fi
  done
  if PLATFORM_LOCK_HELD=1 "$PLATFORMCTL_SCRIPT" validate && PLATFORM_LOCK_HELD=1 "$PLATFORMCTL_SCRIPT" start all && "$PLATFORMCTL_SCRIPT" health true; then
    ...
    PLATFORM_LOCK_HELD=1 "$PLATFORMCTL_SCRIPT" stop all || true; rollback_swaps "$manifest"; PLATFORM_LOCK_HELD=1 "$PLATFORMCTL_SCRIPT" start all || true; printf 'restore validation failed; prior data restored from %s; maintenance mode retained for verification\n' "$rollback_root" >&2; exit 1
  fi
}
